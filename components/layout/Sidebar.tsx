...
const NavItem: React.FC<NavItemProps> = ({ icon, label, href, isActive, onClick, badge }) => {
  return (
    ...
    >
      ...
      {badge && (
        <span className="ml-auto bg-excel-100 text-excel-700 text-xs font-bold px-2 py-0.5 rounded-full">
          {badge}
        </span>
      )}
    </Link>
    ...
    <>
      ...
      <aside className="hidden lg:flex w-64 bg-white border-r border-slate-200 flex-col">
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="bg-gradient-to-br from-excel-50 to-blue-50 rounded-xl p-4 border border-excel-100">
            <div className="flex items-start space-x-2 mb-2">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-excel-600 flex-shrink-0 mt-0.5">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-700 mb-0.5">AI-Powered Extraction</p>
                <p className="text-[10px] text-slate-500 leading-tight">Automatically detect and extract data from images and PDFs</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Mobile Sidebar */}
      <aside
        className={`fixed top-16 left-0 bottom-0 w-64 bg-white border-r border-slate-200 z-40 transform transition-transform duration-300 ease-in-out lg:hidden ${
