...
const HistoryRow = React.memo<HistoryRowProps>(({
  ...
  // 상태별 버튼 텍스트 및 스타일 (모두 "이어가기"로 통일)
  const getActionButton = () => {
    switch (status) {
      case 'completed':
        return {
          text: '이어가기',
          className: 'px-3 py-1.5 bg-excel-50 hover:bg-excel-100 text-excel-700 rounded-lg text-sm font-medium transition-colors'
        };
      case 'pending':
        return {
          text: '이어가기',
